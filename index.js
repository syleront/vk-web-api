...
const request = require("request"); // for partial files uploading

const audioUnmaskSource = require("./lib/audio-unmask-source");
...
let VK = {
  ...
  messageUtils: {
    uploadVideo: (path) => {
      return new Promise((resolve, reject) => {
        needle.post("https://vk.com/al_video.php?act=get_upload_params", {
          al: 1,
          from: "message",
          gid: 0
        }, {
            ...
            if (e) return reject(e);
            let res = JSON.parse(b.match(/<!json>(.+?)$/i)[1]);
            let string_params = querystring.stringify(res.vars);
            let url = res.options.url + "?" + string_params;
            request.post(url, {
              formData: {
    ...
    send: (obj) => {
      return new Promise((resolve, reject) => {
        let peer_id = obj.peer_id ? obj.peer_id : (obj.chat_id ? obj.chat_id + 2e9 : (obj.user_id ? obj.user_id : ""));
        needle.post("https://vk.com/al_im.php", {
          act: "a_start",
          al: 1,
          block: true,
          gid: 0,
          ...
          prevpeer: 0
            ...
            needle.post("https://vk.com/al_im.php", {
              ...
              random_id: getRandomInt(-0xFFFFF, 0xFFFFF),
              to: peer_id
            }, {
                cookies: VK.cookies
              }, (e, r, b) => {
  ...
  videoUtils: { // TODO
    upload: (path, obj) => {
      return new Promise((resolve, reject) => {
        needle.post("https://vk.com/al_video.php", {
          act: "upload_box",
          aid: obj.album_id || 0,
          al: 1,
          from: "video",
          ...
          oid: VK.user_id
            ...
            if (e) return reject(e);
            let params = b.match(/initUpload\('(.+?)',\s?(\{.+?}),\s?(\{.+?}),\s?(\{.+?})/i);
            let upload_params = JSON.parse(params[2]);
            let url = params[1] + "?" + querystring.stringify(upload_params);
            let hashes = JSON.parse(params[4]);
            request.post(url, {
              formData: {
                video_file: fs.createReadStream(path)
              }
              ...
              if (e) return reject(e);
              needle.get(r.headers.location, {
                cookies: VK.cookies
              }, (e, r, b) => {
                let video = JSON.parse(b);
                needle.post("https://vk.com/al_video.php?act=save_video_params", {
                  al: 1,
                  album_id: hashes.aid || obj.album_id || 0,
                  desc: obj.desc || "",
                  hash: hashes.update_db_hash,
                  monetized: "",
                  no_comments: "",
                  oid: upload_params.oid,
                  privacy_video: 0,
                  privacy_videocomm: 0,
                  publish_later: "",
                  repeat: "",
                  ...
                  thumb_hash: "",
                  thumb_id: video.video_id + "_" + video.owner_id,
